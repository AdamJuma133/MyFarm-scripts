...
const Auth = () => {
  ...
  const handleOAuth = async (provider: 'apple' | 'google', label: string) => {
    setError(null);
    ...
    try {
      ...
      if (oauthError) {
        setError(oauthError.message || `Failed to sign in with ${label}`);
      }
    } catch {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-secondary p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl font-bold">{t('app.title', 'MyFarm')}</CardTitle>
          <CardDescription>
        ...
        <CardContent>
          ...
          <form onSubmit={handleSignIn} className="space-y-4">
            ...
            <p className="text-center text-xs text-muted-foreground pt-2">
              {t('auth.accessNote', 'Accounts are provisioned by your MyFarm administrator.')}
            </p>
          </form>
        </CardContent>
      </Card>
